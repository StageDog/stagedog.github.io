...
- 第一条消息: 编写模板文件夹内已经有了 `tavern_sync.mjs` 和 `tavern_sync.yaml`, 你只需要调整 `tavern_sync.yaml` 即可像上文演示的那样用 `node tavern_sync.mjs 命令` 来同步第一条消息
- 脚本: 编写模板支持运行 `pnpm watch` 来实时修改脚本
- 界面: 编写模板支持运行 `pnpm watch` 来实时修改界面
- 世界书: 编写模板文件夹内已经有了 `tavern_sync.mjs` 和 `tavern_sync.yaml`, 你只需要调整 `tavern_sync.yaml` 即可像上文演示的那样用 `node tavern_sync.mjs 命令` 来同步世界书
