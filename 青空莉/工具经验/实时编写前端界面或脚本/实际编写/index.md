...
**以下内容你都可以先用{download}`示例角色卡 <../示例.png>`、`示例/脚本示例` 和 `示例/界面示例` 先过一遍, 别急着搓自己的东西.** \
**以下内容你都可以先用{download}`示例角色卡 <../示例.png>`、`示例/脚本示例` 和 `示例/界面示例` 先过一遍, 别急着搓自己的东西.** \
**以下内容你都可以先用{download}`示例角色卡 <../示例.png>`、`示例/脚本示例` 和 `示例/界面示例` 先过一遍, 别急着搓自己的东西.**
...
你只需要考虑打包结果中的那个 .html 文件, 不必考虑另外的 .map 文件, 它包含了项目中所有文件的代码, 因此你可以将它的内容用代码块包裹 (在之前的{doc}`/青空莉/工具经验/实时编写前端界面或脚本/阅读示例/index`中有解释) 然后复制到酒馆正则中的{menuselection}`替换为`部分; \
如果是脚本, 则应使用打包结果中的那个 .js 文件, 将它的内容复制到酒馆助手脚本库脚本中的{menuselection}`脚本内容`部分.

这样一来, 如果酒馆楼层中存在能被这个酒馆正则匹配的文本, 酒馆正则就会在楼层中替换出代码块, 而酒馆助手会将代码块渲染成前端界面:
