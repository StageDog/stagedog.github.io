...
这个打包结果是单个 html 文件 (如果是脚本则是单个 js 文件), 它包含了项目中所有文件的代码, 因此你可以将它用代码块包裹 (在之前的{doc}`/青空莉/工具经验/实时编写前端界面或脚本/阅读示例/index`中有解释) 然后复制到酒馆正则中的{menuselection}`替换为`部分 (如果是脚本则复制到酒馆助手脚本库脚本中的{menuselection}`脚本内容`部分). \
