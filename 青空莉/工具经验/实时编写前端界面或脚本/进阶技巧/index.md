...
能够获取最新角色卡文件
: 你可以直接像上文我们自动更新前端界面、脚本一样, 使用 jsdelivr 网络链接; 也可以用你自己的云存储链接.
...
为了让这个更新过程更为自动化, 你可以学习{doc}`/青空莉/工具经验/实时编写角色卡、世界书或预设`, 然后调整模板文件夹中的 `tavern_sync.yaml` 配置文件:

- 你可以像`示例/角色卡示例`那样, 在 Cursor 里让 AI 或自己修改角色卡的第一条消息、世界书、版本号等;
- 修改完成后, 你可以不经过酒馆直接打包得到角色卡图片、世界书或预设 JSON 文件;
- 或者, 我已经在模板文件夹配置好了工作流, 当你将更改上传到 github 时, 角色卡、世界书或预设同步脚本会自动打包得到最新的角色卡图片、世界书或预设 JSON 文件.
