...
`界面-发布给玩家` 正则所做的, 就是捕获 AI 回复中的 `[查看日记: 消息]` 部分, 并将其替换为 html 代码块.

:::{figure} 界面-发布给玩家正则.png
:::

回过头来, 为什么打开`界面-发布给玩家`正则后会收到整整三条弹窗提示呢? 因为我们的消息楼层中总共有三个 `[查看日记: 消息]`, 正则会依次捕获替换, 而酒馆助手由此渲染了三个前端界面:
...
前端界面能做的不仅仅是显示而已, 你还可以和它交互而执行对应的代码. 例如, 我们点击示例中的前端界面, 将会发现自己发送了一条消息, 并且输入框右下角显示出正在生成的图标:
