...
- {lolodesu_path}`青空莉调整后的日记络络 <src/日记络络>`
...
但是像好感度一类变量, 我们可能需要更详细的解释其意义; 想必你见过我的角色阶段写法: \
(只要用了 `associated variable` 或 `stage name` 的一般是源自我的写法, 虽然我很想吐槽啥都不改、连我为什么列这些都不知道就直接抄.)
...
- 将变量列表放在 D4 等位置, 然后精心设计变量更新格式及分析思维链提示词, 来避免 AI 误解
- 取消对变量更新的“仅格式提示词”正则, 把变量更新情况发给 AI, 这样 AI 能知道变量更新过了
...
变量更新规则没必要放在 D1 (论为什么我讨厌 MVU 示例卡, 以及不自己理解提示词写法直接照抄提示词), 你完全可以将它置于 D4 等位置, 让 AI 在输出变量更新时先在思维链中回想`检查`、分析变量更新, 再实际输出更新命令.
...
但一般而言, 你会用上我在 [2024/11/21](https://discord.com/channels/1134557553011998840/1308984948794982421) 设计的一类变量更新格式提示词. 它最初的版本其实没有复杂思维链, 只在尾部有一个 `原因` 帮助 AI 提高注意力:
...
```{code-block} yaml
:caption: 回忆变量更新所需的`检查`, 据此分析该如何更新
...
在思维链中让 AI 回忆`检查`规则, 据此分析该如何更新变量:
...
在思维链中不是让 AI 回忆`检查`, 而是让它原封不动地列举所有`检查`内容, 再进行分析:
...
由于思维链中列举`检查`和基于`检查`进行分析放置地过于紧密, AI 可能偷懒, 则你可以将这两句话隔离一下:
...
变量更新格式说白了也是{doc}`/青空莉/工具经验/提示词个人写法/额外输出格式/index`, 因此完全可以放在 D4 而在 D1/D0 用变量更新格式强调来保证 AI 输出.
