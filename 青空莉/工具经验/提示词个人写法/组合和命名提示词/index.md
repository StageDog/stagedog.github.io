...
- 支持将正文压缩成一个消息里的 A 和 B 对话, 自建对话语境, 免受模型有关 user 和 ai 对话理解的影响从而避免模型关于 AI 和用户对话的影响.
...
具体地, 在 YAML 中, 我们可以用 `&名字` 来将某个部分设置成锚点, 而用 `*名字` 来在别的地方使用这个锚点: (不用担心使用锚点时轻微的语法错误, AI 不在乎)
