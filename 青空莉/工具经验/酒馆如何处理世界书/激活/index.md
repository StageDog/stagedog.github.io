...
如何告知绿灯该这么做呢? 我们展开条目, 在{guilabel}`主要关键字 (primary keyword)`中填入`络络`:
...
为此, 我们可以用英文输入法下的逗号 `,` (半角逗号) 来间隔这些文本, 将它们全都填入{guilabel}`主要关键字`中, 即`络络,女皇,笨蛋`; 当然你也可以加空格显得好看一些: `络络, 女皇, 笨蛋`.
...
影响界面显示
: 比如让你在聊天界面中看不到思维链. 这由正则设置中的{guilabel}`仅格式显示`控制.
