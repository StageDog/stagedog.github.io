...
- 打开{menuselection}`世界书编辑界面`并完整截图条目列表, 尤其是变量列表、变量更新规则、变量输出格式;
- 打开{menuselection}`酒馆助手-脚本`并完整截图脚本列表, 尤其是角色脚本;
- 打开{menuselection}`正则`并完整截图正则列表, 尤其是局部正则;
- 如果和 AI 回复相关, 点击楼层右上角的铅笔, 完整截图 AI 的回复内容;
