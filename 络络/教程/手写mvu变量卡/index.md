...
- **重复一遍, 变量提示词的写法只取决于你的想象**, 甚至不必拘泥于放在 `<update>` 中, 因为 MVU 脚本会读取 AI 回复和用户输入中任意位置的 `_.set(...)`
- 你也可以在开局消息中写 `_.set(...)`, 来为不同开局设置不同的初始变量
- AI 输出的 `<update>` 没必要再发给 AI, 因为已经被 MVU 脚本使用了, 而 AI 在后续回复中不需要参考它.
...
- 你可以不在变量列表中列出某些变量, 只要求 AI 更新它, 但不要求 AI 了解它现在的值是多少.
- 你没必要非要用 `<StatusPlaceHolderImpl/>` 来显示状态栏, 你可以用其他任何方式来显示它. 你可以自己定义一个输出格式, 在里面要求 AI 输出周围地点等细节信息, 而用酒馆正则捕获这个格式, 替换为带变量的状态栏.
