...
但实际上在正文中我们是使用的 `export const Schema = z.looseObject({...})` 而不是 `export const Schema = z.object({...})`. 这是为什么呢?

因为:

- 除了我们的角色卡外, 还可能存在其他变量结构脚本, 比如某个全局世界书也设计了一套变量;
- 我们可能想把角色卡的变量拆分到多个变量结构脚本里.

也就是说, 最顶层的 `const Schema = z.object({...})` 除了必须有我们自己设计的那一套变量外, 还要能允许其他变量存在. 因此我们用 `z.looseObject({...})` 而不是 `z.object({...})`:

- `z.object({...})` 用于规定变量能接收一个对象 (文件夹), 而这个对象内视为只有 `{...}` 指定的那些字段 (文件或子文件夹), 其他字段视为不存在;
- `z.looseObject({...})` 用于规定变量能接收一个对象 (文件夹), 而这个对象内必须有 `{...}` 指定的那些字段 (文件或子文件夹), **但其他字段也允许存在**.

**总之, 只要记得使用 `export const Schema = z.looseObject({...})` 而不是 `z.object({...})` 即可.**
...
在文本变量部分提到的 `.or(...)` 和 `z.union([...])` 并不只能用于文本, 你可以规定一个变量既能接收数值, 又能接收`'待初始化'`:
