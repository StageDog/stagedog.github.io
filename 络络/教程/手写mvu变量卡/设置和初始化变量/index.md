...
如果日志查看器中没有显示`【脚本|变量结构】: 变量结构注册成功`、或者显示黄色的`SCHEMA VIOLATION`
: 说明你的变量结构脚本出现了错误. \
  请从日志查看器复制`【脚本|变量结构】`开头的所有日志, 将它发给 AI 来让它重新生成变量结构脚本; 没有日志就直接告诉 AI 出错了需要改. \
  如果实在不能解决, 请通过帖子询问青空莉qwq.

新开聊天、发送任意消息, 如果出现`[MVU]变量初始化失败`或`[MVU]变量更新失败`黄框提示:
: 说明你的变量结构脚本出现了错误. \
  请从日志查看器复制`【脚本|变量结构】`开头的所有日志, 将它发给 AI 来让它重新生成变量结构脚本; 没有日志就直接告诉 AI 出错了需要改. \
  如果实在不能解决, 请通过帖子询问青空莉qwq.

新开聊天、发送任意消息, 如果出现`[MVU zod]变量初始化失败`或`[MVU zod]变量更新失败`黄框提示:
